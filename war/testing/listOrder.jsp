...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list-table">
  ...
  <tr>
  	<td width="20%"><%= printer.print("Date") %></td>
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="0" class="list-table">
  <tr>
  	<td width="20%"><a href="JavaScript:doMenu('<%=branch.getBranchName()%>');" id='x<%=branch.getBranchName()%>'>[+]</a><%= branch != null ? printer.print("Branch") + ": " + branch.getBranchName() + " (" + orderArrayList.size() + ")": printer.print("Inexistent Branch") %></td>
  	<td width="10%"></td>
  	<td width="40%"></td>
  	<td width="10%"></td>
    <td width="10%"></td>
...
<div  class="treeview" id='<%=branch.getBranchName()%>' >
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list-table">	
		...
		%>
		  <tr>
		  	<td width="20%"><%= DateManager.printDateAsString(order.getOrderTime()) %></td>
		    <td width="10%"><%= order.getKey() %></td>
		    <td width="40%"><%= customer != null ? customer.getCustomerName() : "Inexistent customer" %></td>
		    <td width="10%"><%= printer.print(order.getOrderTypeString()) %></td>
		    <td width="10%"><%= printer.print(order.getOrderStatusString()) %></td>
		    <td width="10%">
		    <% 
		    ...
		    %>
		    	<a id="<%= order.getKey() %>" href="/restaurant/editOrder.jsp?k=<%= order.getKey() + "&readonly=true" %>"><%= printer.print("View") %></a>
		    	<br/>
		    	<a href="javascript:void(0);" onclick="confirmDelete('/manageRestaurant?action=delete&type=O&k=<%= order.getKey() %>', 'Order');"><%= printer.print("Delete") %></a>
		    <%
		    }
		    %>
		    </td>
