...
public class UserManager {
	...
	public static User getUser(String email) {
		...
		User user;
		try {
			user = pm.getObjectById(Administrator.class, key).getUser();
		} 
		catch (JDOObjectNotFoundException e) {
			user = null;
		}

        // if it is not an admin lets look for a restaurant
        if (user == null) {
            key = KeyFactory.createKey(Restaurant.class.getSimpleName(), email);
            try {
                user = pm.getObjectById(Restaurant.class, key).getUser();
            } 
            catch (JDOObjectNotFoundException e) {
                user = null;
            }
        ...
        // if it is not an admin or a restaurant could it be a customer?
        if (user == null) {
            key = KeyFactory.createKey(Customer.class.getSimpleName(), email);
            try {
                user = pm.getObjectById(Customer.class, key).getUser();
            } 
            catch (JDOObjectNotFoundException e) {
                user = null;
            }
