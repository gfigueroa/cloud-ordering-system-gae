...
if (type == 'A') {
%>
	<%= printer.print("Active Stamps") %>
<%
}
else if (type == 'I') {
%>
	<%= printer.print("Inactive Stamps") %>
<%
}
else {
%>
	<%= printer.print("Stamps History") %>
<%
}
%>
</h1>
...
<div>
	<a href="listStamps.jsp?r_key=<%= KeyFactory.keyToString(restaurantKey) %>&type=A"><%= printer.print("Active Stamps") %></a> | <a href="listStamps.jsp?r_key=<%= KeyFactory.keyToString(restaurantKey) %>&type=I"><%= printer.print("Inactive Stamps") %></a> | <a href="listStamps.jsp?r_key=<%= KeyFactory.keyToString(restaurantKey) %>&type=H"><%= printer.print("Stamps History") %></a>
</div>
...
%>
  <tr>
  	<td width="20%"><a href="addStamps.jsp">+ <%= printer.print("Add Stamps") %></a></td>
  	<td width="20%"></td>
    <td width="20%"></td>
    <td width="10%"></td>
    <td width="10%"></td>
...
%>
  ...
  <tr>
    ...
    <td width="20%"><%= printer.print("Stamps Title") %></td>
    <td width="10%"><%= printer.print("Purchase amount") %></td>
    <td width="10%"><%= printer.print("No. Product Items") %></td>
  ...
  %>
	  <tr>
	    ...
	    <td width="20%"><%= restaurantNews.getRestaurantNewsTitle() %></td>
	    <td width="10%"></td>
	    <td width="10%"></td>
	    ...
	    if (type == 'A' || type == 'I') {
	    %>
	    	<a id="<%= KeyFactory.keyToString(restaurantNews.getKey()) %>" href="editStamps.jsp?k=<%= KeyFactory.keyToString(restaurantNews.getKey()) + "&readonly=true" %>"><%= printer.print("View") %></a>
	    	<br/>
	    	<a href="editStamps.jsp?k=<%= KeyFactory.keyToString(restaurantNews.getKey()) %>"><%= printer.print("Edit") %></a>
	    <%
	    }
	    else {
	    %>
	    ...
	    %>
	    <br/>
	    <a href="javascript:void(0);" onclick="confirmDelete('/manageRestaurant?action=delete&type=N&k=<%= KeyFactory.keyToString(restaurantNews.getKey()) %>', '<%= printer.print("Are you sure you want to delete this Stamp")%>');"><%= printer.print("Delete") %></a>
	    </td>
	  </tr>
  <%  
  }
